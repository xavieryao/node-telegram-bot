      ...
      if (body.ok) {
        body.result.forEach(function (result) {
          if (result.update_id >= self.offset) {
            ...
            var command = result.message.text.split(' ', 2)[0];
            if (command.charAt(0) === '/') {
	            command = command.replace(/[^a-zA-Z0-9 ]/g, "");
	            //TODO add multiple arguments recognition
	            if (result.message.text.split(' ', 2)[1])
	            { 
		            self.emit(command, result.message.text.split(' ', 2)[1], result, self);
	            } else {
		            self.emit(command, result.message.text, result, self);
	            }
            }
